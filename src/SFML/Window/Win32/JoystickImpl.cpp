...
void JoystickImpl::updateConnectionsDInput()
{
    ...
    // Enumerate devices
    HRESULT result = directInput->EnumDevices(DI8DEVCLASS_GAMECTRL, reinterpret_cast< LPDIENUMDEVICESCALLBACKW >( &JoystickImpl::deviceEnumerationCallback ), NULL, DIEDFL_ATTACHEDONLY);
...
bool JoystickImpl::openDInput(unsigned int index)
{
    ...
    for (std::vector<JoystickRecord>::iterator i = joystickList.begin(); i != joystickList.end(); ++i)
    {
        if (i->index == index)
        {
            ...
            result = m_device->EnumObjects( reinterpret_cast< LPDIENUMDEVICEOBJECTSCALLBACKW >( &JoystickImpl::deviceObjectEnumerationCallback ), this, DIDFT_AXIS | DIDFT_BUTTON | DIDFT_POV);
